...
def gbn_receiver(receiver_host_name, receiver_listening_port):
    ...
    while True:
        ...
        received_raw_packet, client_info = receiver_socket.recv(4096)
        ...
        computed_checksum.update(received_seq_number)
        computed_checksum.update(payload)
        computed_checksum.update(mss)
        computed_checksum.update(seq_bits)
        ...
        if received_checksum == computed_checksum.digest():
            if received_seq_number == expected_seq_num:
                ...
                ack_checksum.update(expected_seq_num)
